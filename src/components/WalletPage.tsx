  ...
  componentDidMount() {
    getBiot(async (biot: any) => {
      let upd = async () => {
        ...
        let objTransactions = await biot.core.getWalletTransactions(
          this.props.walletId
        );
        let myAddresses = await biot.core.getAddressesInWallet(
          this.props.walletId
        );
        let listTransactions = this.calcListTransactions(
          objTransactions,
          myAddresses
        );
  ...
  showChannels = () => {
    return this.state.channels.map((channel: IChannels) => {
      return (
        <div
          key={channel.id}
          className={"channels-list-body"}
          onClick={() => this.showModalChannel(channel.row)}
        >
          <div className={"channels-list-body-name"}>
            {channel.id.substr(0, 20) + "..."}
          </div>
          <div className={"list-body-balance-ch"}>
            My amount: {channel.myAmount} | Peer amount: {channel.peerAmount}
          </div>
        </div>
      );
    });
  ...
  render() {
    let getBlock = () => {
      if (this.state.list == "transactions") {
        return (
          <div key={"transactions"} className={"transactions"}>
        ...
        return (
          <div>
            ...
            >
              <div>
                ...
                <span style={{ fontSize: "12px" }}>
                  {this.state.modalChannelId}
                </span>
              </div>
              <div>Status: {this.state.modalChannelStatus}</div>
              {this.state.modalChannelStatus === "Open" ? (
                <div>
                  <button
            ...
            <div
              ...
              onClick={() => this.setState({ isShowModalChannel: false })}
            >
              _
            </div>
          </div>
